...
from catalog.api.utils.throttle import ThrottleExemption, ExemptionAwareThrottle, InternalNetworkExemption, ApiKeyExemption
...
def redis(monkeypatch) -> FakeRedis:
    ...
    monkeypatch.setattr('catalog.api.utils.throttle.get_redis_connection', get_redis_connection)
...
def assert_throttles(view, request, times=4):
    assert_view_consistent_status_code(view, request, expected_status=429)


def assert_does_not_throttle(view, request, times=4):
    assert_view_consistent_status_code(view, request, expected_status=200)
...
def test_internal_network_exemption_throttles_when_ip_not_in_allowlist(redis, request_factory):
    view = get_throttled_view(MockInternalNetworkExemptThrottle)
    request = request_factory.get('/')
    assert not redis.sismember(InternalNetworkExemption.redis_set_name, request.META["REMOTE_ADDR"])
    assert_throttles(view, request)


@pytest.mark.django_db
def test_api_key_exemption_passes_when_token_in_allowlist(redis, access_token, authed_request):
...
@pytest.mark.django_db
def test_api_key_exemption_throttles_when_token_not_in_allowlist(redis, access_token, authed_request):
    view = get_throttled_view(MockApiKeyExemptThrottle)
    client_id, _, _ = get_token_info(access_token.token)
...
def test_multiple_exemptions_allows_if_one_passes_api_key(redis, access_token, authed_request):
    ...
    client_id, _, _ = get_token_info(access_token.token)
    redis.sadd(ApiKeyExemption.redis_set_name, client_id)
    assert not redis.sismember(InternalNetworkExemption.redis_set_name, authed_request.META["REMOTE_ADDR"])
    assert_does_not_throttle(view, authed_request)

@pytest.mark.django_db
def test_multiple_exemptions_allows_if_one_passes_internal_network(redis, access_token, authed_request):
    view = get_throttled_view(MockMultipleExemptionThrottle)
    client_id, _, _ = get_token_info(access_token.token)
    redis.sadd(InternalNetworkExemption.redis_set_name, authed_request.META["REMOTE_ADDR"])
    assert not redis.sismember(ApiKeyExemption.redis_set_name, client_id)
    assert_does_not_throttle(view, authed_request)


@pytest.mark.django_db
def test_multiple_exemptions_throttles_if_none_pass(redis, access_token, authed_request):
    view = get_throttled_view(MockMultipleExemptionThrottle)
    client_id, _, _ = get_token_info(access_token.token)
    assert not redis.sismember(InternalNetworkExemption.redis_set_name, authed_request.META["REMOTE_ADDR"])
    assert not redis.sismember(ApiKeyExemption.redis_set_name, client_id)
