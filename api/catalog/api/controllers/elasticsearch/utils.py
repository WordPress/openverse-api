...
def exclude_filtered_providers(s: Search) -> Search:
    ...
    if filtered_providers is None:
        ...
        logger.debug(
            'adding filtered providers to cache'
        )
        cache.set(
...
def paginate_with_dead_link_mask(
    ...
    # TODO: What does this request_size mean vs the one below that doesn't subtract 1 from the page?
    # Why do we subtract 1 here and the other time we do not?
    ...
    logger.debug(
        f'accu_query_mask={accu_query_mask} '
    )
    start = 0
    if page > 1:
        caught = False
...
def get_query_slice(
    ...
    if start_slice + end_slice > ELASTICSEARCH_MAX_RESULT_WINDOW:
        raise ValueError(f"Deep pagination is not allowed. Window totalled {start_slice + end_slice}.")
