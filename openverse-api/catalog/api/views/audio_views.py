...
{SearchMedia.search_description}"""  # noqa
    ...
    @swagger_auto_schema(
        ...
        code_examples=[{"lang": "Bash", "source": audio_search_curl}],
    )
...
{RelatedMedia.recommendations_read_description}"""  # noqa
    ...
    def get(self, request, identifier, format=None):
        related, result_count = search_controller.related_media(
            uuid=identifier, index="audio", request=request, filter_dead=True
        )

        context = {"request": request}
        serialized_related = AudioSerializer(related, many=True, context=context).data
        response_data = {
...
class AudioWaveform(APIView):
    ...
    def get(self, request, identifier, format=None):
        ...
        try:
            ...
            return Response(
                status=500, data={"message": "It's not you, it's me.", "error": str(e)}
            )
        finally:
