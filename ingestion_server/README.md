...
This codebase is deployed as Docker image to the GitHub container registry [ghcr.io](https://ghcr.io). The deployed image is then pulled in the production environment. See the [`publish.yml`](../.github/workflows/publish.yml) workflow for deploying to GHCR.

The images used to be published to Dockerhub. The previous Docker hub image is at [openverse/ingestion_server](https://hub.docker.com/r/openverse/ingestion_server) but the image previously lived at [creativecommons/ingestion_server](https://hub.docker.com/r/creativecommons/ingestion_server).
